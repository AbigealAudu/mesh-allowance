...
(define-map transaction-history
  { owner: principal, spender: principal, transaction-id: uint }
  {
    amount: uint,
    timestamp: uint,
...
(define-private (update-spent-amount 
  ...
  (let (
    (current-allowance (unwrap-panic 
      (map-get? allowances { owner: owner, spender: spender })
    ))
...
(define-private (log-transaction 
  ...
  (let (
    (transaction-count (default-to u0 (get-last-transaction-id owner spender)))
    (new-transaction-id (+ transaction-count u1))
...
(define-private (get-last-transaction-id (owner principal) (spender principal))
  (fold 
    (lambda (transaction (max-id (optional uint)))
      (let (
        (current-id (get transaction-id transaction))
      )
        (if (or (is-none max-id) (> current-id (unwrap-panic max-id)))
          (some current-id)
          max-id
      )
    )
    (filter 
      (lambda (transaction) 
        (and 
          (is-eq (get owner transaction) owner)
          (is-eq (get spender transaction) spender)
        )
      )
      (map-keys transaction-history)
    )
    none
  )
)
...
;; Spend from an existing allowance
(define-public (spend-allowance 
  ...
  (begin
    (try! (check-spending-authorization owner tx-sender amount))
    (try! (update-spent-amount owner tx-sender amount))
    (try! (log-transaction owner tx-sender amount description))
    (ok true)
  )
)
...
(define-public (modify-allowance 
  ...
  (let (
    ...
    (current-rules (unwrap! 
      (map-get? allowance-rules { owner: tx-sender, spender: spender }) 
      ERR-NOT-AUTHORIZED
    ))
  )
...
(define-read-only (get-allowance-details (owner principal) (spender principal))
  (map-get? allowances { owner: owner, spender: spender })
)
